...
class PlayState extends MusicBeatState {
	...
	public static var ratingStuff:Array<Dynamic> = [
		['Brain-Dead!', 0.2], // From 0% to 19%
		['Tone-Deaf!', 0.4], // From 20% to 39%
		['Pretty Shit', 0.5], // From 40% to 49%
		['Rusty', 0.6], // From 50% to 59%
		['Unlucky', 0.69], // From 60% to 68%
		['Nice', 0.7], // 69%
		['Mediocre', 0.8], // From 70% to 79%
		['Passable', 0.9], // From 80% to 89%
		['Talented!', 1], // From 90% to 99%
		['Pitch-Perfect!!', 1] // The value on this one isn't used actually, since Perfect is always "1"
	];
	...
	public var gfSpeed:ShortUInt = 1;
	...
	override public function create() {
		...
		Lib.application.window.title = "FNF: Abandoned by Disney - " + SONG.song;
		...
		timeBar = new FlxBar(timeBarBG.x + 4, 0, LEFT_TO_RIGHT, FlxG.width, 5, this,
			'songPercent', 0, 1);
		...
		scoreTxt.setFormat(Paths.font("EnchantedLand-jnX9.ttf"), 20, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
	...
	public function createRuntimeShader(name:String):FlxRuntimeShader {
		...
		//trace('Creating Runtime Shader $name');
	...
	public inline function addBehindGF(obj:FlxObject) {
		insert(members.indexOf(gfGroup), obj);
	}

	public inline function addBehindBF(obj:FlxObject) {
		insert(members.indexOf(boyfriendGroup), obj);
	}

	public inline function addBehindDad(obj:FlxObject) {
	...
	public function updateScore(miss:Bool = false) {
		...
		if (songScore == 0){
			...
			Lib.application.window.title = "FNF: Abandoned by Disney - " + SONG.song + " [ " + 'Score: ' + songScore + '  |  Misses: ' + songMisses + '  |  Rating: ' + ratingName + (ratingName != '?' ? ' (${Highscore.floorDecimal(ratingPercent * 100, 2)}%)   $ratingFC' : '') + " ]";
		}
	...
	private function generateSong():Void {
		...
		function addNote(note:Note) {
			note.ID = spawnNoteID;
			// allNotes[spawnNoteID] =
			unspawnNotes[spawnNoteID] = note;
			spawnNoteID++;
			return note;
		}

		var holdOffset = (Conductor.stepCrochet / FlxMath.roundDecimal(songSpeed, 2));
	...
	override public function update(elapsed:Float) {
		...
		if (FlxG.keys.pressed.SHIFT && !endingSong && !inCutscene){
			if(chartingMode){chartingMode = false;}
			else{chartingMode = true;}
			trace('charting is toggled to ' + chartingMode);
			//lazy charting code so I can debug stuff faster
		}
		...
		if (camZooming && !isDead) {
			var lerpVal = CoolUtil.boundTo(1 - (elapsed * 3.125 * camZoomingDecay * playbackRate), 0, 1);
		...
		if (generatedMusic && !inCutscene) {
			...
			if (startedCountdown) {
				...
				notes.forEachAlive(function(daNote:Note) {
					...
					var angleDir = strumDirection * flixel.math.FlxAngle.TO_RAD;
					if (daNote.copyAngle)
		...
		if (((skipHealthCheck && instakillOnMiss) || health <= 0) && !practiceMode && !isDead) {
			...
			if (ret != FunkinLua.Function_Stop) {
				...
				/*boyfriend.cameras = [camOther];
					camOther.setPosition(boyfriend.x - (boyfriend.width / 2), boyfriend.y - (boyfriend.height / 2)); */
				// FlxG.camera.setPosition(boyfriend.x - (boyfriend.width / 2), boyfriend.y - (boyfriend.height / 2));
				boyfriend.getScreenPosition(_point);
	...
	public function endSong():Void {
		...
		if (achievementObj != null) {
			...
			var achieve:String = checkForAchievement([
				'week1_nomiss', 'week2_nomiss', 'week3_nomiss', 'week4_nomiss', 'week5_nomiss', 'week6_nomiss', 'week7_nomiss', 'ur_bad', 'ur_good', 'hype', 'two_keys', 'toastie', 'debugger']);
		...
		if (ret != FunkinLua.Function_Stop && !transitioning) {
			...
			if (isStoryMode) {
				...
				if (storyPlaylist.length <= 0) {
					...
					FlxG.sound.playMusic(Paths.music('NEW_freeplay_Theme'));
	...
	private function popUpScore(note:Note):Void {
		...
		var daRating:Rating = Conductor.judgeNote(noteDiff / playbackRate);

		totalNotesHit += daRating.ratingMod;
		note.ratingMod = daRating.ratingMod;
		if (!note.ratingDisabled)
			daRating.increase();
		note.rating = daRating.rating;
		...
		if (showComboNum) {
			...
			if (lastScore != null) {
				while (lastScore.length > 0) {
					var score = lastScore.pop();
					score.destroy();
					comboLayer.remove(score, true);
				}
			}
	...
	private function onKeyPress(event:KeyboardEvent):Void {
		...
		var key = getKeyFromEvent(eventKey);
	...
	private function getKeyFromEvent(key:FlxKey):ByteInt {
		if (key != NONE) {
			for (i in 0...keysArray.length) {
	...
	function noteMissPress(direction:ByteInt = 1):Void // You pressed a key when there was no notes to press for this key
	{
		...
		for (i in 0...achievesToCheck.length) {
			...
			if (!Achievements.isAchievementUnlocked(achievementName) && !cpuControlled) {
				...
				switch (achievementName) {
					...
					case 'toastie': if (/*ClientPrefs.framerate <= 60 &&*/ !ClientPrefs.shaders && ClientPrefs.lowQuality && !ClientPrefs.globalAntialiasing) {
							unlock = true;
						}
