...
class CreditsState extends MusicBeatState {
	...
	override function create() {
		...
		Lib.application.window.title = "FNF: Abandoned by Disney - Cast Members";

		persistentUpdate = true;
		bg = new FlxSprite().loadGraphic(Paths.image('cast_members'));
		add(bg);
		bg.screenCenter();

		var creditsImage = new FlxSprite().loadGraphic(Paths.image('CreditsImage'));
		creditsImage.screenCenter();
		creditsImage.setGraphicSize(Std.int(creditsImage.width * .4));
		add(creditsImage);
