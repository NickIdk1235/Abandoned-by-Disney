...
class FunkinLua {
	...
	public static var currentScript:FunkinLua = null;
	...
	public function new(script:String) {
		...
		Lua_helper.add_callback(lua, "setShaderProperty", function(obj:String, variable:String, value:Dynamic) {
			...
			if (killMe.length > 1) {
				setVarInArray(getPropertyLoopThingWhateverObject(shader, killMe), killMe[killMe.length - 1], value);
				return true;
			}
			setVarInArray(shader, variable, value);
			return true;
			#else
			luaTrace("setShaderProperty: Platform unsupported for Runtime Shaders!", false, false, FlxColor.RED);
			return false;
			#end
		});
		Lua_helper.add_callback(lua, "getShaderProperty", function(obj:String, variable:String) {
			#if (linc_luajit >= "0.0.6")
			var shader:FlxRuntimeShader = getShader(obj);
			if (shader == null)
				return null;
			...
			var killMe:Array<String> = variable.split('.');
			if (killMe.length > 1) {
				return getVarInArray(getPropertyLoopThingWhateverObject(shader, killMe), killMe[killMe.length - 1]);
			}
			return getVarInArray(shader, variable);
			#else
			luaTrace("getShaderProperty: Platform unsupported for Runtime Shaders!", false, false, FlxColor.RED);
			return null;
			#end
		});
		...
		Lua_helper.add_callback(lua, "characterDance", function(character:String) {
			switch (character.toLowerCase()) {
				...
				case 'gf' | 'girlfriend': if (PlayState.instance.gf != null)
		...
		Lua_helper.add_callback(lua, "close", function() {
			return currentScript.closed = true;
		});
		...
		Lua_helper.add_callback(lua, "characterPlayAnim", function(character:String, anim:String, ?forced:Bool = false) {
			...
			switch (character.toLowerCase()) {
				case 'dad': if (PlayState.instance.dad.animOffsets.exists(anim))
						PlayState.instance.dad.playAnim(anim, forced);
				case 'gf' | 'girlfriend': if (PlayState.instance.gf != null && PlayState.instance.gf.animOffsets.exists(anim))
						PlayState.instance.gf.playAnim(anim, forced);
				default: if (PlayState.instance.boyfriend.animOffsets.exists(anim))
	...
	public static function getVarInArray(instance:Dynamic, variable:String):Any {
		//trace('getVarInArray: ' + variable + " | " + instance + " | " + currentScript.scriptName);
		var shit:Array<String> = variable.split('[');
		if (shit.length > 1) {
			var blah:Dynamic = null;
			if (PlayState.instance.variables.exists(shit[0])) {
	...
	function getGroupStuff(leArray:Dynamic, variable:String) {
		...
		if (killMe.length > 1) {
			...
			switch (Type.typeof(coverMeInPiss)) {
				case ValueType.TClass(haxe.ds.StringMap) | ValueType.TClass(haxe.ds.ObjectMap) | ValueType.TClass(haxe.ds.IntMap) | ValueType.TClass(haxe.ds.EnumValueMap): return coverMeInPiss.get(killMe[killMe.length - 1]);
				default: return Reflect.getProperty(coverMeInPiss, killMe[killMe.length - 1]);
			};
		}
		switch (Type.typeof(leArray)) {
			case ValueType.TClass(haxe.ds.StringMap) | ValueType.TClass(haxe.ds.ObjectMap) | ValueType.TClass(haxe.ds.IntMap) | ValueType.TClass(haxe.ds.EnumValueMap): return leArray.get(variable);
	...
	public static function getPropertyLoopThingWhateverObject(obj:Dynamic, killMe:Array<String>, ?getProperty:Bool = true):Dynamic {
		var end = killMe.length;
		if (getProperty)
			end = killMe.length - 1;
