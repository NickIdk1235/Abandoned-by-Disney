...
-- INITIAL FUNCTIONS
local function tableCopy(t, st, copyMeta, x)
	if (copyMeta == nil) then
		copyMeta = true
	end
	x = x or 0
	getfenv().things = getfenv().things or {}
	local things = getfenv().things
	if (things[t] ~= nil) then
		return things[t]
	end
...
local function mathlerp(from, to, i)
...
local function initializeCams()
	local function cam(t, cam, class)
		t.getProperty = type(class) == "string" and function(v)
			return getPropertyFromClass(class, cam .. "." .. v)
		end or function(v)
			return getProperty(cam .. "." .. v)
		end
		t.setProperty = type(class) == "string" and function(v, x)
			return setPropertyFromClass(class, cam .. "." .. v, x)
		end or function(v, x)
			return setProperty(cam .. "." .. v, x)
		end
...
local function updateCam(t, dt)
	if (not cams.isReady) then
		return
	end
	...
	if (t.fxShakeDuration > cool) then
		...
		if (config.useScrollForShake and t == cams.game) then
			t.scrollOffset:set(rX, rY)
		else
			t.viewOffset:add(rX * t.zoom, rY * t.zoom)
		end
...
local function updateCamEarly(t, dt)
	if (t == cams.game) then
...
local function updateCamsEarly(dt)
	if (not dead()) then
